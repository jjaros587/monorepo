...
import { OrderArgs, OrderDirection } from 'src/graphql'
  ...
  return (
    <Icon
      ...
      onClick={() =>
        setOrder({
          orderBy: column.key,
          orderDirection: order?.orderDirection === OrderDirection.Asc ? OrderDirection.Desc : OrderDirection.Asc
        })
      }
    />
  )
...
export const TableHeader = <T extends { _id: string }>({ data, columns, order, setOrder }: TableHeaderProps<T>) => {
