...
interface EntitySelectFieldDescriptor extends SelectFieldBaseDescriptor {
  ...
  entityName: string
  itemToPair: (item: any) => { value: any; label: any }
}
...
type SimpleFormField = EmailFieldDescriptor | PasswordFieldDescriptor | TextFieldDescriptor | NumberFieldDescriptor
...
export function isSimpleField(field: FormFieldDescriptor): field is SimpleFormField {
  return [FormFieldTypes.Text, FormFieldTypes.Email, FormFieldTypes.Password, FormFieldTypes.Number].includes(
    field.type
  )
}
