import { DocumentNodeService, LocalStorageService, EntityManagerService, ModalManager } from '@services'
...
class ServiceContainerClass {
  ...
  register(serviceName: string, instance: any) {
    console.assert(!(serviceName in this.services), `Service with name "${serviceName}" is already registered`)
