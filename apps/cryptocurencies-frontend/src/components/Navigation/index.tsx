...
import { NavigationNode, RouteType } from 'src/app/router/ScreenDescriptor'
    ...
    Object.values(SCREEN_DESCRIPTORS).forEach((descriptor) => {
      if (descriptor.navigationNode) {
        if (
          ...
          navigationItems.push({ ...descriptor.navigationNode, route: descriptor.route })
        }
      }
    })
    ...
    <S.Navigation opened={opened}>
      ...
      <S.Bottom>
        {user ? (
          ...
          <NavLink key={SCREEN_DESCRIPTORS['login'].route} to={SCREEN_DESCRIPTORS.login.route} icon={'login'} />
        )}

        <NavItem icon={opened ? 'arrowDoubleLeft' : 'arrowDoubleRight'} onClick={() => setOpened(!opened)} />
      </S.Bottom>
    </S.Navigation>
  )
}
