...
export const useListingResolver = (size?: number, number?: number): ListingResolver => {
  const [pageNumber, setPageNumber] = useState(number || 1)
  const [additionalPages, setAdditionalPage] = useState(0)
  const [limit, setLimit] = useState(size || 10)
  const [order, setOrder] = useState<OrderArgs | undefined>({ orderBy: 'date', orderDirection: OrderDirection.Desc })
